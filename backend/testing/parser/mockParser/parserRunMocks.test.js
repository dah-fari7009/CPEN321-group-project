...
test('Invalid parse text', () => {
        return parserMocks.mockParse({body: {userID: 0, text: parserMocks.invalidParseText}}, {body: null})
                .catch((error) => {
                        expect(error).toEqual({err: "No \\end{presentation} token found after \\begin{presentation}."});
                });
})
...
test('Sample unparse', () => {
	...
	var actualOut = parserMocks.mockUnParsePresentation({body: parserMocks.sampleUnparseInput});
